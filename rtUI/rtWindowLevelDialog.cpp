...
void rtWindowLevelDialog::setImageData(vtkImageData* dat) {
  ...
  if (dat->GetScalarType() == VTK_UNSIGNED_CHAR) {
    ...
    for (int ix1=0; ix1<dims[0]*dims[1]*dims[2]; ix1++) {
      ...
      spot = floor(loc);
      if ((spot >= 0) && (spot < c_histogramSize)) {
        m_histogram[spot]++;
      } else {
  ...
  } else if ((dat->GetScalarType() == VTK_UNSIGNED_SHORT) || (dat->GetScalarType() == VTK_SHORT)) {
    ...
    cout << "WARNING: W/L dialog only works with uchar8, uint16, sint16 (" << dat->GetScalarTypeAsString() << ")" << endl;
